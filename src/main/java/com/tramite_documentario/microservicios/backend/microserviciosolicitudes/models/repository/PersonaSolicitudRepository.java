...
public interface PersonaSolicitudRepository extends PagingAndSortingRepository<PersonaSolicitud, Long> {
    @Query("select ps from PersonaSolicitud ps join fetch ps.rol r join fetch ps.persona join fetch ps.solicitud s where r.id=1 and s.id=?1")
    public PersonaSolicitud findPersonaSolicitudEmisorBySolicitud(Long idSolicitud);

    @Query("select ps from PersonaSolicitud ps join fetch ps.rol r join fetch ps.persona p join ps.solicitud s where r.id=2 and s.id=?1")
